...
If your system Go version is older than 1.21, install a newer version:
```sh
# Download and extract Go 1.23.0
mkdir -p ~/sdk
curl -L https://go.dev/dl/go1.23.0.linux-amd64.tar.gz | tar -C ~/sdk -xz
mv ~/sdk/go ~/sdk/go1.23.0
...
```sh
cd fwdays_homeworks/timoni
timoni mod init my-web-app
```
